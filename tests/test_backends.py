...
def create_build_graph(compiler, linker):
    ...
    libmath = LinkedNode(os.path.join(PATHS["build"], "libmath.so"), [factorial_o, fibonacci_o], linker=linker, internal_path=os.path.join(PATHS["build"], "libmath.so"), flags=flags+BuildFlags()._enable_shared(), libs=["stdc++"])
    test = LinkedNode(os.path.join(PATHS["build"], "test"), [test_o, libmath], linker=linker, internal_path=os.path.join(PATHS["build"], "test"), flags=flags, libs=["stdc++", "math"], lib_dirs=[os.path.dirname(libmath.path)])
