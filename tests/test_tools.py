...
class TestCompilers(unittest.TestCase):
    ...
    def compile_src(self, filename, compiler):
        ...
        compiler.compile(input_file, output_file, include_dirs=self.include_dirs, opts=["--std=c++17"])
        return output_file
    ...
    def test_can_compile_and_link_so(self):
        for comp in TestCompilers.compilers:
            with self.subTest():
                input_files = [self.compile_src("factorial", comp), self.compile_src("fibonacci", comp)]

        for link in TestCompilers.linkers:
            with self.subTest():
                out_so = os.path.join(TEST_PROJECT_BUILD, "libs", "libmath.so")
                link.link(input_files, out_so, opts=["--std=c++17"], shared=True)
                self.assertTrue(os.path.exists(out_so))

    def test_can_compile_and_link_executable(self):
        for comp in TestCompilers.compilers:
            with self.subTest():
                input_files = [self.compile_src("factorial", comp), self.compile_src("fibonacci", comp), self.compile_test("test", comp)]

        for link in TestCompilers.linkers:
            with self.subTest():
                out_exec = os.path.join(TEST_PROJECT_BUILD, "libs", "test")
                link.link(input_files, out_exec, opts=["--std=c++17"])
